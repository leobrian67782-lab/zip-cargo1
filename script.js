...
function trackShipment() {
  ...
  result.style.cssText = 'display:block;background:white;border-radius:16px;box-shadow:0 8px 32px rgba(0,0,0,0.10);';
  result.innerHTML = `
    <div style="display:flex;flex-direction:column;align-items:center;padding:52px 20px;gap:18px;font-family:'Outfit',sans-serif;">
      <div style="position:relative;width:72px;height:72px;">
        ...
        <div style="position:absolute;inset:0;border-radius:50%;border:5px solid transparent;border-top-color:#e8920a;animation:zcSpin 0.85s linear infinite;"></div>
        <div style="position:absolute;inset:11px;border-radius:50%;background:#0d1f35;display:flex;align-items:center;justify-content:center;">
          <i class="fa-solid fa-truck" style="color:#e8920a;font-size:18px;"></i>
        </div>
      </div>
      <div style="font-weight:700;color:#0d1f35;font-size:1.05rem;">Locating shipment…</div>
      <div style="font-size:0.85rem;color:#7a9ab8;">Searching for <strong style="color:#0d1f35;">${input}</strong></div>
    </div>`;

  if (!document.getElementById('zcSpinStyle')) {
    const _st = document.createElement('style');
    _st.id = 'zcSpinStyle';
    _st.textContent = '@keyframes zcSpin{to{transform:rotate(360deg)}}';
