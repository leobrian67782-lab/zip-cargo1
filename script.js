// ===== API PROXY =====
// All data calls go through proxy.php — the JSONBin key
// never appears in any browser-visible file.
const ZC_PROXY = 'proxy.php';

// ===== RATE LIMITER (client-side, backs up server-side limit in proxy.php) =====
const _trackAttempts = { count: 0, resetAt: 0 };
function checkTrackRateLimit() {
  const now = Date.now();
  if (now > _trackAttempts.resetAt) {
    _trackAttempts.count = 0;
    _trackAttempts.resetAt = now + 60000;
  }
  _trackAttempts.count++;
  return _trackAttempts.count <= 10;
}

async function getShipmentsPublic() {
  try {
    const res  = await fetch(ZC_PROXY + '?action=getShipments');
...
async function saveInquiryPublic(inquiry) {
  try {
    await fetch(ZC_PROXY + '?action=saveInquiry', {
      method:  'POST',
