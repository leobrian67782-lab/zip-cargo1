// ===== JSONBIN CONFIG =====
// NOTE: For production, move this key to a backend proxy endpoint
...
// Bin IDs hardcoded — same as admin.js
var ZC_SHIPMENTS_BIN = '69fbdb43adc21f119a6408ab';
var ZC_INQUIRIES_BIN = '69fbdbd4adc21f119a640d71';

// ===== RATE LIMITER (prevents tracking spam) =====
const _trackAttempts = { count: 0, resetAt: 0 };
function checkTrackRateLimit() {
  const now = Date.now();
  if (now > _trackAttempts.resetAt) {
    _trackAttempts.count = 0;
    _trackAttempts.resetAt = now + 60000; // 1 minute window
  }
  _trackAttempts.count++;
  return _trackAttempts.count <= 10; // max 10 lookups per minute
}

async function getShipmentsPublic() {
  try {
    const res  = await fetch(ZC_JSONBIN_BASE + '/b/' + ZC_SHIPMENTS_BIN + '/latest', {
      headers: { 'X-Master-Key': ZC_JSONBIN_KEY }
    });
    const data = await res.json();
    return (data.record && data.record.shipments) ? data.record.shipments : [];
  } catch { return []; }
}

async function saveInquiryPublic(inquiry) {
  try {
    const res  = await fetch(ZC_JSONBIN_BASE + '/b/' + ZC_INQUIRIES_BIN + '/latest', {
      headers: { 'X-Master-Key': ZC_JSONBIN_KEY }
    });
    const data = await res.json();
    const list = (data.record && data.record.inquiries) ? data.record.inquiries : [];
    list.push(inquiry);
    await fetch(ZC_JSONBIN_BASE + '/b/' + ZC_INQUIRIES_BIN, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', 'X-Master-Key': ZC_JSONBIN_KEY },
      body: JSON.stringify({ inquiries: list })
    });
...
async function trackShipment() {
  ...
  const trackBtn  = document.querySelector('.tracking-form button');
  ...
  // Validate tracking number format (basic sanitization)
  if (!/^[a-zA-Z0-9\-_]{3,30}$/.test(input)) {
    result.className = 'track-result error';
    result.innerHTML = 'Invalid tracking number format. Please check and try again.';
    return;
  }

  // Rate limiting
  if (!checkTrackRateLimit()) {
    result.className = 'track-result error';
    result.innerHTML = '<i class="fa-solid fa-triangle-exclamation"></i> Too many requests. Please wait a moment and try again.';
    return;
  }

  // Show loading state
  result.className = 'track-result loading-state';
  result.innerHTML = `
    <div class="track-loading">
      <div class="track-loading-spinner">
        <div class="spinner-ring"></div>
        <div class="spinner-ring spinner-ring-2"></div>
        <i class="fa-solid fa-satellite-dish spinner-icon"></i>
      </div>
      <div class="track-loading-text">Searching for your shipment...</div>
      <div class="track-loading-sub">Connecting to tracking network</div>
      <div class="track-loading-dots"><span></span><span></span><span></span></div>
    </div>
  `;
  if (trackBtn) { trackBtn.disabled = true; trackBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Tracking...'; }

  // Minimum loading time for UX polish
  const [shipments] = await Promise.all([
    getShipmentsPublic(),
    new Promise(resolve => setTimeout(resolve, 1800))
  ]);

  if (trackBtn) { trackBtn.disabled = false; trackBtn.innerHTML = 'Track Now'; }

  const s = shipments.find(s => s.tracking.toLowerCase() === input.toLowerCase());

  if (!s) {
    result.className = 'track-result error';
    result.innerHTML = `No shipment found for <strong>${sanitize(input)}</strong>. Please check your tracking number and try again.`;
    // Hide map if visible
