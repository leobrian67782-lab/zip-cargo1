...
// ===== LOGIN =====
// ── Wake-up helper: pings /health until server responds or timeout ────────────
async function waitForServer(maxWaitMs = 70000) {
  ...
  while (Date.now() - started < maxWaitMs) {
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (msg) msg.textContent = 'Server ready!';
...
async function adminLogin() {
  ...
  // Step 1: Quick check — is the server already awake?
  ...
  try {
    ...
    clearTimeout(t);
    serverReady = r.ok;
  } catch (e) { /* not awake yet */ }

  // Step 2: If not awake, show progress bar and wait
  if (!serverReady) {
    btn.textContent = 'Waking server…';
    serverReady = await waitForServer(70000);
    if (!serverReady) {
      error.textContent = 'Server is taking too long to start. Please refresh the page and try again.';
  ...
  // Step 3: Server is awake — attempt login
  ...
  try {
    ...
    if (data.ok) {
      ...
      error.textContent = data.error || 'Incorrect username or password.';
      setTimeout(() => error.textContent = '', 4000);
    }
  } catch (e) {
    error.textContent = 'Login request failed. Please try again.';
