...
async function adminLogin() {
  ...
  // Attempt login with one automatic retry (handles Render cold-start wake-up)
  const MAX_ATTEMPTS = 2;
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      if (attempt > 1) {
        error.textContent = 'Server is waking up, please wait…';
      ...
      // Final failure
      error.textContent = 'Connection error — server may be starting up. Please try again in 30 seconds.';
      setTimeout(() => error.textContent = '', 8000);
    }
  }
